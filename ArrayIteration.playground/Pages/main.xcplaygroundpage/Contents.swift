 ...
 */
func greetall(_ nlist:[String]) {
    for namex in nlist {
        print ("Good morning \(namex)")
    }
}


greetall(["sam","young","paul"]);
 ...
 */
func trim50(_ nlist:[Int])->[Int] {
    var new_list : [Int] = []
    for intx in nlist {
        if (intx<50) {
        new_list.append(intx)
        }
    }
    return new_list
}

var newn:[Int] = trim50([12,67,11,9,0,70,16])
