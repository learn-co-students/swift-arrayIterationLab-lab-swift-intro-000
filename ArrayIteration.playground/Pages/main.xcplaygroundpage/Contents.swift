 ...
 */
func morningGreeting(names: [String])-> Void {
    for name in names {
        if name == "Michael" {
 ...
 */
func integersUnderFifty (integers: [Int])-> [Int] {
    //create and initialize a new array of ints
    var integersLessThanFifty = [Int]()
    
    //iterate over the array that was passed into the function
    for integer in integers {
        //add the value to integersLessThanFifty if condition is satisfied
        if integer < 50 {
            integersLessThanFifty.append(integer)
        }
    }
    return integersLessThanFifty
}
